...
def segment_image(
    ...
    Returns:
        ...
        colored_mask: The pink and light blue binary mask
    """
    ...
    binary_mask = (seg_map > 0).astype(np.uint8)
    ...
    # Create pink and light blue colored mask instead of black and white
    # Pink for background (0), Light blue for segments (1)
    pink = [165, 115, 115]  # Pink color in RGB
    light_blue = [170, 180, 255]  # Light blue color in RGB
    
    colored_mask = np.zeros((binary_mask.shape[0], binary_mask.shape[1], 3), dtype=np.uint8)
    colored_mask[binary_mask == 0] = pink  # Pink for background
    colored_mask[binary_mask == 1] = light_blue  # Light blue for segments
    
    # Create colored mask for original size
    colored_mask_original = np.zeros((original_h, original_w, 3), dtype=np.uint8)
    colored_mask_original[binary_mask_original == 0] = pink  # Pink for background
    colored_mask_original[binary_mask_original == 1] = light_blue  # Light blue for segments
    
    # Save results
    base_name = os.path.splitext(os.path.basename(image_path))[0]
    
    # Save pink-light blue mask
    cv2.imwrite(
        os.path.join(output_dir, f"{base_name}_colored_mask.png"), 
        cv2.cvtColor(colored_mask_original, cv2.COLOR_RGB2BGR)  # Convert back to BGR for OpenCV
    )
    ...
    plt.subplot(1, 3, 2)
    plt.title('Pink & Light Blue Mask')
    ...
    return seg_map, colored_mask

if __name__ == "__main__":
    # Configuration - update these paths according to your setup
    IMAGE_PATH = "C:/Users/Shikaka/Desktop/Screenshot 2025-04-11 215738.png"
    ...
    # Run segmentation
    seg_map, colored_mask = segment_image(
